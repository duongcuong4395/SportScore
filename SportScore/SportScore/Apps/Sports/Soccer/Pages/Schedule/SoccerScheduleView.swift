...
struct SoccerScheduleView: View {
    ...
    var body: some View {
        VStack {
            if scheduleVM.modelsForNext.count > 0 {
                Text("Upcoming")
                    .font(.callout.bold())
                SoccerScheduleListItemView(models: scheduleVM.modelsForNext)
            }
            if scheduleVM.modelsForPrevious.count > 0 {
                Text("Results")
                    .font(.callout.bold())
                SoccerScheduleListItemView(models: scheduleVM.modelsForPrevious)
            }
        }
    }
}
